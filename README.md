<h1 align="center">
Hi, I'm Omar Aguilar 👋
	<a href="https://github.com/OmarAguilar43">
		<img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30">
	</a>
</h1>

<p align="center">
	<a href="https://github.com/OmarAguilar43">
		<img src="https://komarev.com/ghpvc/?username=TU_USERNAME&label=Profile%20views&color=0e75b6&style=flat" />
	</a>
	<a href="https://github.com/OmarAguilar43">
		<img src="https://img.shields.io/github/followers/TU_USERNAME?label=Followers" />
	</a>
</p>

<p align="center">
	<img src="https://readme-typing-svg.herokuapp.com?lines=Software+Engineering+Student;Full+Stack+Developer;NestJS+%7C+Next.js+%7C+Vue.js;TypeScript+Focused;Building+real-world+projects;Always+learning&center=true&width=520&height=45">

</p>

---

<pre>
💻 Software Engineering student focused on Full Stack development
🏗️ Experience building real-world academic and personal projects
🧠 Strong interest in backend architecture, APIs and scalable systems
🌐 Frontend experience with React, Next.js and modern UI patterns
🌱 Currently learning Vue.js and modern frontend patterns (Composition API, state management)
🗄️ Comfortable working with SQL & NoSQL databases
🚀 Main stack: TypeScript, NestJS, Next.js, React, MongoDB, SQL
📚 Continuously improving data modeling, system design & clean architecture
🤝 Open to internships, junior roles and collaborative projects
</pre>

## 🌱 Currently Learning
- Vue.js (Composition API, reactive state, component patterns)
- State management patterns with Pinia
- Microservices architecture with NestJS (modular services, communication patterns)

---

## 🤝 Connect with me
<p align="center">
	<a href="mailto:omaroaguilar43@gmail.com">
		<img src="https://img.shields.io/badge/gmail-%23EA4335.svg?style=plastic&logo=gmail&logoColor=white" />
	</a>
	<a href="https://www.linkedin.com/in/omar-aguilar-b9078b343">
		<img src="https://img.shields.io/badge/linkedin-%230A66C2.svg?style=plastic&logo=linkedin&logoColor=white" />
	</a>
	<a href="https://github.com/OmarAguilar43">
		<img src="https://img.shields.io/badge/github-%23181717.svg?style=plastic&logo=github&logoColor=white" />
	</a>
</p>

---

## 🛠️ Tech Stack

### 👨‍💻 Programming Languages
<p>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/Python-14354C?logo=python&logoColor=white" />
</p>

### 🧰 Frameworks & Libraries
<p>
  <img src="https://img.shields.io/badge/NestJS-E0234E?logo=nestjs&logoColor=white" />
  <img src="https://img.shields.io/badge/Next.js-000000?logo=nextdotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/Vue.js-4FC08D?logo=vue.js&logoColor=white" />
  <img src="https://img.shields.io/badge/Pinia-FADA5E?logo=pinia&logoColor=black" />
  <img src="https://img.shields.io/badge/Prisma-2D3748?logo=prisma&logoColor=white" />
  <img src="https://img.shields.io/badge/TailwindCSS-38B2AC?logo=tailwindcss&logoColor=white" />
</p>


### 🗄️ Databases & Tools
<p>
  <img src="https://img.shields.io/badge/MongoDB-47A248?logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-336791?logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL%20Server-CC2927?logo=microsoftsqlserver&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white" />
</p>

### 💻 Software & Utilities
<p>
  <img src="https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Postman-FF6C37?logo=postman&logoColor=white" />
  <img src="https://img.shields.io/badge/VSCode-007ACC?logo=visualstudiocode&logoColor=white" />
</p>

---

## 📊 GitHub Stats
<p align="center">
  <img width="49%" src="https://github-readme-stats.vercel.app/api?username=TU_USERNAME&show_icons=true&theme=default" />
  <img width="49%" src="https://github-readme-streak-stats.herokuapp.com/?user=TU_USERNAME" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=TU_USERNAME&layout=compact&langs_count=8" />
</p>

---

## 🚀 Let’s build something meaningful
If you’re interested in collaboration, internships, or junior opportunities,  
feel free to reach out — I’m always open to learning and building better software.

